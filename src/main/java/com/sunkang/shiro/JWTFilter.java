...
public class JWTFilter  extends BasicHttpAuthenticationFilter {
    ...
    protected boolean executeLogin(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String tokenStr = (String)request.getAttribute("token");
    ...
    private void response401(ServletRequest req, ServletResponse resp,Exception e) {
            try {
                req.setAttribute("errorClass",e.getClass().toString());
                req.setAttribute("errorCode",e.getMessage());
                req.getRequestDispatcher("/server/shiroError").forward(req,resp);
            } catch (Exception e1) {
                e1.printStackTrace();
            }
    }
